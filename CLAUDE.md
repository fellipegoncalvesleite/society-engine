...
Last updated:
  ...
  PROGRESS — THE PHYSICAL CHAIN IS CLOSED FOR SCOUT/PROBE, ONE SEAM REMAINS DEFERRED /
  ROADMAP DOES NOT ADVANCE. Branch checkpoint/resource-investigation-physical-26, continuing
  ...
  **MEASURED AT THE EXACT SEAM, NOT ACROSS A STEP.** The pre-existing audit-only
  ...
  192 selections (91.7%) gained target-area knowledge, with 0 execution identities in
  existence. After: 0 of 234, with 234 of 234 carrying an exact execution identity.** The after
  arm selects MORE investigations (234 vs 192) — this is NOT reduced scout frequency dressed as
  a repair. The 2 residual after-arm changes are knowledge being LOST to ring eviction, the
  ...
  **NATURAL OCCURRENCE, 343 selections (20 y x 3 scenarios x 2 seeds):** 139
  ...
  support. `beyond_same_day_reach` being the largest class is the HONEST refusal: selection
  reaches 10 tiles (`SCOUT_MAX_DISTANCE`), an honest same-day round trip covers 8
  (`SAME_DAY_ROUND_TRIP_TILE_BUDGET`), so anything past 4 one way is outside same-day physics.
  ...
  **THREE INSTRUMENT ERRORS IN THIS PASS'S OWN PROBES ARE RECORDED, NOT DROPPED.** (1) The
  ...
  return kinds, hardship outcome, expedition knowledge latency, fixtures P1-P13 35/35 plus a
  negative control.
  ...
  **DEFERRED AND UNPROVEN — DO NOT CITE AS A FINDING.** The gap between selection reach (10) and
  the fixed same-day budget (8) refuses 43% of natural investigations. Whether that is a defect
  — a fixed budget ignoring `bandMobility`'s dynamic pace/conditioning/fatigue — or an honest
  physical limit is NOT established; no constant in the checkpoint's forbidden list was touched
  and no counterfactual was run. Likewise `route_unavailable` (54/343) has no failure memory.

  NOT RUN, deliberately: no 200 y / 500 y matrix, no population/survival/fitness comparison, no
  Crowding work. No claim that physical investigation improves outcomes.
...
Current active checkpoint:
  RESOURCE INVESTIGATION / TEMPORARY USE (roadmap item 2) — CORRECTION-26 is PROGRESS, not
  closed. Crowding (item 3) is NOT authorized and was not begun. Historical context follows:
