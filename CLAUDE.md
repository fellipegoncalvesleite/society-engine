...
| --- | --- | --- |
| ROADMAP ITEM 5 — PHYSICAL-EFFECT PROVENANCE PASS 2 (2026-08-18; `checkpoint/item5-physical-effect-provenance-pass2`; production `2d7251d4456ba52e98b6ebaddd51c54a5f5575f9`; **IMPLEMENTED / ITEM 5 OPEN / NOT FROZEN / NOT PUSHED**) | Added explicit per-variant execution provenance and one central causal gate in `practicalResponses.ts`: `practice_only` remains live; `existing_physical_work` requires persisted execution authority; `material_execution_required` cannot create physical coefficients or response-bound efficacy maturation without execution proof. Persisted `waterWorks` remains authoritative. Planned experiment materials/procedure/cost wording was corrected without adding inventory/crafting state. | Portable-material execution authority still does not exist and is deliberately not fabricated here. Pass 3 and Item-5 freeze are unstarted. Historical Pass-1 source-hash audit is expected to reject the changed `practicalResponses.ts` hash while all live authority checks remain true. |
