...
Last updated:
  2026-07-25 (REPEATED BAND EXPANSION + HABITAT VIABILITY CORRECTION-14: PROGRESS — NOT
  ACCEPTED / DO NOT MERGE, branch checkpoint/repeated-band-expansion-fission-14 from public
  main 668763f (CORRECTION-13, integrated into main at the start of this checkpoint; local
  and remote main both equal 668763f). The ANNUAL demographic step read the INSTANTANEOUS
  nutrition state, so it sampled the same seasonal trough every year: the richest map2
  catchment averaged 1.73x demand while the demographic read logged 0.79 mean food stress,
  with 359 physically-surplus years and 1 surplus-signal year. Two food-access defects
  compounded it — the same-day trip argmax ran over every distance while the caller discards
  out-of-budget winners, and a saturated 48-slot resource-memory cap evicted every newly
  observed patch, so a relocating band could not learn local ground (17-season zero-trip runs;
  141/480 zero-support seasons). Fixes: deriveAnnualNutritionState (one annual call site;
  seasonal read retained for behavior), a requireSameDay selection domain with domain-aware
  reachability, and cap protection for just-observed patches. Rich lineages 0 -> 4 median
  successful fissions / 149 people; Map 1 defaults 0 -> 8 fissions / 65 -> 337 people / no
  founder extinct. NOT accepted: no second-generation fission and four other gates unmet,
  traced to a measured 9-tile destination-knowledge horizon. See §23 and
  docs/evidence/correction14/.)
...
Current active checkpoint:
  DESTINATION KNOWLEDGE HORIZON / EXPLORATION REACH — named by CORRECTION-14 as the
  measured blocker for repeated multi-generation expansion. A founder's known-tile horizon
  never exceeds 9 tiles (0 known tiles beyond 10 with usable confidence at every 50-year
  sample to y300), so every fission daughter is necessarily founded inside the parent's own
  ~10-tile foraging catchment, both halves draw on the same patches, and a lineage saturates
  one region (mean support 0.87 by Year 500) instead of populating country. Widening it means
  changing exploration/expedition reach, which CORRECTION-14 deliberately did not redesign.
  CLIMATE / WEATHER / REGIONAL SEASONALITY FOUNDATION-2 remains the standing roadmap item
  after it. Adaptation is reached ONLY through src/sim/agents/adaptationBoundary.ts; the
  seasonal read-model rebuild budget is 2 full buildTickContextCache + 1 partial refresh per
  tick; climate must replace/extend src/sim/agents/environmentBoundary.ts, never bypass it.
  NOTE the annual/seasonal nutrition split introduced by CORRECTION-14: demography reads
  deriveAnnualNutritionState; every behavioral consumer keeps deriveCanonicalNutritionState.
...
| --- | --- | --- |
| REPEATED BAND EXPANSION + HABITAT VIABILITY CORRECTION-14 (2026-07-25; branch `checkpoint/repeated-band-expansion-fission-14` from public main `668763f`; **PROGRESS — DO NOT MERGE**) | **The annual demographic step read the seasonal trough, and two food-access defects kept bands out of country they were standing in.** (1) Demography runs ONCE A YEAR (`shouldRunAnnualDemography`, spring) and integrates a year, but read the INSTANTANEOUS `deriveCanonicalNutritionState`; because the annual step always lands on the same season it sampled the same phase every year — the lean one. Richest map2 catchment, one year: support ratios 2.03/1.65/1.06/**0.09** (physical harvest spring 61, summer 46, autumn 31, winter 2.8); over 500 years mean annual support **1.73×** demand against a demographic read of **0.79** mean food stress, **359 physically-surplus years, 1 surplus-signal year** (`recoveryRelief` is a TRAILING streak that a lean trailing season zeroes, and it gates CORRECTION-13's `nutritionalSurplus`). Fix: `deriveAnnualNutritionState` (`seasonalSurvival.ts`) replaces exactly the two instantaneous terms with four-season counterparts (mean seasonal food stress; share of the year's seasons meeting the SAME `isRecoverySeason` condition the streak uses) and is consumed by ONE call site, `computeBandDemography` — every behavioral consumer keeps the seasonal read. (2) `applyTripDay` discards candidates that miss the same-day budget but `selectTripCandidate` ran its argmax over every distance, so with ONE candidate per trip day an out-of-budget winner wasted the day: added a `requireSameDay` domain (mirror of CORRECTION-4's expedition fix) plus domain-aware `hasReachablePatchMemory`, and local reconnaissance now bootstraps on no REACHABLE memory rather than no memory at all. (3) `enforceResourceKnowledgeCap` ranked a saturated 48-slot cap purely by retention, so a fresh first-sight memory always lost and a relocated band could never learn local ground — a permanent learning lock; it now retains just-observed patch ids, evicting the lowest-retention OLDER belief. Measured: 17-season `trips: 0` runs in the highest-stock country on the map; zero-support seasons **141/480 → 1/480**. Plus a step-mode repair in `expedition.ts` (recon returns stamped observations from the batch-start world time). Results: rich **0 → 4 median successful fissions**, 5 living bands, **149 people** from 34, mean rate −0.0008 → **+0.0044**; ordinary founder survival 0.6 → **1.0**; Map 1 defaults 3 → **13 living bands**, 65 → **337 people**, 0 → **8 fissions**, 2 → **0 extinct founders**; Map 2 defaults 4 → **10 bands**, 98 → **277 people**, corridors still extinct. Fission + cohort conservation 0 mismatches. Added `lineageExpansionAudit.mjs`, `canonicalStateFingerprint.mjs`, `foodAccessCollapseProbe.mjs`, `destinationKnowledgeHorizonProbe.mjs`, `expansionPerformanceProbe.mjs`, audit-only `diagnostics/fissionDiagnostics.ts`; graph node `annualNutritionRead` | **NOT ACCEPTED.** Five gates unmet: no rich seed reaches 6 successful fissions (max 4), **no second-generation fission**, only 2/5 `good` lineages fission successfully, marginal-but-escapable never escapes, hostile does not go extinct. Four share one measured cause: a band's known-tile horizon **never exceeds 9 tiles**, so every daughter is founded inside the parent's own catchment and the lineage saturates one region (mean support 0.87 by Y500). That is exploration/logistical reach, upstream of fission, deliberately not redesigned. Hostile fails differently — tiers are defined by the STARTING catchment and a mobile band legitimately drifts out of one. Also open: `deriveKnownBandSpacingForFission` excludes the fissioning band itself from every crowding term (drafted, measured, reverted for no effect); fission regenerates cohorts from a canonical ratio instead of transferring members (population exact, identity not); `demographicDeathMemoryPathAudit` FAILS 2/11 confounded net-rate orderings, isolated to the annual read and left unfixed rather than tuned away |
...
Record the **destination knowledge horizon / exploration reach** architecture when that
checkpoint is explicitly begun and accepted. CORRECTION-14 measured it as the binding limit
on repeated multi-generation expansion: a band never learns country beyond its own ~10-tile
foraging catchment (max known-tile distance 7–9 at every sample over 300 years), so fission
cannot disperse a lineage across the landscape no matter how the fission target is scored.
Do not treat it as a fission defect and do not fold it into a demographic checkpoint.

Historical note, retained: core pipeline consolidation is complete (DECOMPOSITION-1/-2/-3 all
recorded above). Record **expeditionary logistical mobility / task camps / viewshed /
