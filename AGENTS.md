...
Current active checkpoint: CORRECTION-24A FINALIZATION — EVENT-PAIRED FIRST-READER TRACE (branch checkpoint/ordinary-exploration-capacity-24, continuing 4c44079; CORRECTION-23 FROZEN at 59391d54; local main untouched at 668763f, remote main untouched at 0a43083). DIAGNOSTIC ONLY — diagnostics-off fingerprints IDENTICAL to d865beec on both maps. **THIS PASS CORRECTS THE PREVIOUS ONE AND THE CAUSE IS THE INSTRUMENT, NOT THE SIMULATION.** `tileObservation.ts:326-329` overwrites `acquisition` on every observation unless the record already reads `residential_observation`, so an exploration-learned tile STOPS carrying its label the moment the band residentially observes it — and the old E6 selected rows BY that label. It is renamed **GLOBAL-SNAPSHOT SENSITIVITY**. **WITHDRAWN: 'the effect fades with time', 'compounding is refuted', 'knowledge is worth less than the labour it consumes', 'O2 makes outcomes worse'.** **The same bias applies to the O5 arms** (`getFissionTargetRecordIds` suppresses on the same field), so their byte-identical results are NOT evidence of inert readers. **THE EVENT-PAIRED TRACE, tracking by `recordEventId` and stripping ALL FIVE stores the §5 inventory proved can name a tile, finds 4,637 of 19,974 records (23.2%) changing a reader's SELECTED ACTION** — movement 1,339, camp 446, resource 1,692, fission 1,160 — 3,743 within 90 days, 0 never read, 0 evicted before read, positive controls 96%/99.6%/27%/100%. **BUT movement probe soundness is 59.7% and only 508 of 1,339 changes are physically realised, so 2.5% is a FLOOR with a closed chain and 23.2% a CEILING of sensitivity — READER VALUE IS NOT CLAIMED PROVEN.** O2 paired mediation: +6/-5/=44, median 0, bootstrap [-0.127, +0.291] crossing zero. X3 is NON-VACUOUS (9/9 bands, no heading, motive present); **X1-X16 = 16 PASS / 0 FAIL / 0 VACUOUS**. **NOT RUN: the 200/500-year event-paired matrices — gate 12 UNMET.** See docs/evidence/correction24a/COMPLETION_FINDINGS.md sections F1-F6.
