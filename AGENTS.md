...
Current active checkpoint: RESOURCE INVESTIGATION PHYSICAL EXECUTION — CORRECTION-26 — PASS, TECHNICALLY COMPLETE, AWAITING HUMAN ROADMAP CLOSURE (branch checkpoint/resource-investigation-physical-26, continuing its own architecture-decision commit b746b68; CLOSURE-25 FROZEN at f947550; local and remote main untouched at 0a43083). **PRODUCTION BEHAVIOUR CHANGED — no fingerprint parity is claimed or possible.** Entry invariant PASSED: `git diff --exit-code f947550 -- src/sim` clean. **A selected `resource_scout`/`logistical_probe` no longer observes anything.** `collectProbeObservationTargets` is deleted; selection observes what a `stay` observes and leaves ONE bounded `PendingInvestigationRecord` carrying the exact `Decision.id` into the following season's first trip day, where `agents/intraSeasonTrips.ts` staffs a party from the labour the day's foraging group left, walks the same `buildOutboundPathTiles`/`findPassablePath` route, and either arrives or names why not. Observation happens ONLY on arrival, through the canonical `observeTileAndNearby`. **Measured at the pre-existing audit-only `decisionObserver` seam (unchanged at f947550): target-area knowledge gained at selection 176/192 (91.7%) → 0/234, on MORE selections, not fewer.** PENDING IS NOT EXECUTED and the two are named apart: 234 selections produce 234 exact PENDING identities (`selectionsWithPendingIdentity` — the check at the decision seam, which proves joinability and nothing physical), resolving to **97 physical executions + 132 named non-executions + 5 still awaiting a trip day = 234**. 343 natural selections resolve 139 executed / 147 `beyond_same_day_reach` / 54 `route_unavailable` / 3 pending / 0 lost / 0 duplicate executions / 0 information receipts. `TemporaryTaskCampRecord` → `TemporaryTaskPartyRecord`, written only on a real departure (`camp_movement_temporary_record` 129 → 0; `expedition_task_camp` 103 → 113, untouched and NOT merged). 0 runtime cycles in `src/sim`, 0 `agents → rules` runtime edges. **A real step-mode regression was introduced and fixed** (the executor observed with `runDailyActions`' span-start `world.time`) and the fixture that passed while it was live was strengthened with a negative control. **INHERITED, NOT A REGRESSION:** `expeditionLifecycleAudit` FAILS here and identically on f947550. **DEFERRED AND UNPROVEN:** `beyond_same_day_reach` is an honest named refusal under the currently authoritative production boundary; **the four-tile boundary itself is NOT claimed proven physically correct**. The possible mismatch with dynamic `bandMobility` is unproven in either direction, no mobility constant was changed, no separate correction is authorized, and it is not part of CORRECTION-26. NOT RUN: long-horizon matrix, population/survival comparison, Crowding. **STATUS: PASS — TECHNICALLY COMPLETE / AWAITING HUMAN ROADMAP CLOSURE / DO NOT MERGE.** Roadmap item 2 closes on the supervising human review, not on this pass's authority; item 3 Crowding is NOT started and no CORRECTION-27 exists. See docs/evidence/resource-investigation-physical-26/FINDINGS.md.
