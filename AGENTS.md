...
Current active checkpoint: ORDINARY EXPLORATION AUTHORITY / LAUNCH-FUNNEL DIAGNOSIS — CORRECTION-24A is PROGRESS — ORDINARY-EXPLORATION BLOCKER ISOLATED / NO PRODUCTION REPAIR YET, and PARTIAL: gates 13-18 are UNMET (branch checkpoint/ordinary-exploration-capacity-24 from CORRECTION-23's closure 59391d54; CORRECTION-23 is FROZEN and its remote still resolves to 59391d54 exactly; local main untouched at 668763f, remote main untouched at 0a43083). DIAGNOSTIC ONLY — diagnostics-off canonical fingerprints IDENTICAL to 59391d54 (map1 7239c085…, map2 d748c78a…). **THE BLOCKER IS NOT MOTIVE, DIRECTION, LABOUR OR COMPETITION.** Over 277,250 real production launch opportunities (11 worlds x 5 seeds x 40y, one typed primary blocker each): eligibility is **99.4% on nine of eleven worlds** and 80.1%/91.2% on the default maps, mean evidence score 0.55/0.66 against a 0.50 threshold with NOTHING below 0.3 on map1; `headingAvailable` is **1.000 on every world** (corridor_continuation 83,173 / water_margin 24,531 / known_edge 750 on map2, mean 43.3 known frontier-edge tiles); `INSUFFICIENT_LABOR` = **0**; `ACTIVE_CAP_FULL` = 32; and BOTH displacement classes together are **176 (0.06%)** — exploration does not lose contests. **THE TWO REAL BLOCKERS: (a) `ALREADY_EXPLORING` 89.12%** — FRONTIER_EXPLORATION_SUPPRESSION_TICKS = 12 ticks = 12 seasons = **3 SIMULATED YEARS**, stamped when the party is RAISED not when it returns, so a band gets one look per three years whatever it found (measured: 13 launches per 40-year run, one per 3.1 years — exact); **(b) `VALID_BUT_IDLE_SLOT_UNUSED` 8.60%, of which 23,852 of 23,852 were NEVER OFFERED the slot and 0 were offered-and-refused** — exploration is called only when no retrieval/patch-verification/reconnaissance candidate exists, and the claiming family then launches nothing (patch_verification 13,447, reconnaissance 4,621, retrieval 440). **TERRAIN-HETEROGENEOUS**: nine worlds are bound by the window, but **site_B_dry_plains and site_D_aquatic are bound by the ordering gate** — valid proposal on 59%/56% of opportunities, ~7,000 claimed-and-unlaunched each, and they launch LEAST (27 vs 63-65). **§15 class I (interaction-dependent) = C (a cap) on nine worlds + E (idle-capacity policy) on two.** A, B and D are REFUTED. **A FIRST VERSION OF THIS AUDIT REPORTED `OFF_LAUNCH_CADENCE` AT 83% — that was the audit's own sampling choice** (production schedules one day in six); off-cadence days are no longer recorded and every denominator is real opportunities. **NOT BUILT, gates unmet: O0-O5 counterfactuals, X1-X16 fixtures, 200y/500y horizons, the dc08b2d historical comparison, feedback-loop first-divergence traces, the read-only projection, and every E4/E5/E6 returned-knowledge measurement** — so returned records, one-year eviction, first readers and changed physical actions are NOT measured, and **no repair is licensed**: shortening the window is not justified while the CORRECTION-23E retention debt could erase every extra return. PASSED: tsc, build, graph 221/764, import/adaptation/decision boundaries, anti-omniscience all-zero, determinism true, step-mode PASS, food capture 1.000, per-lineage and cohort conservation, diagnostics-off fingerprint parity to 59391d54. 26.8 ms/tick. See docs/evidence/correction24a/. Previous context: TEMPORARY-USE PENDING-ACTION AUTHORITY / EXACT LAUNCH-TO-CAMP CONSUMPTION — CORRECTION-23J is PASS — TEMPORARY-USE SUSPENDED UNTIL A REAL OPERATION READER EXISTS / CORRECTION-23 VERIFICATION BEHAVIOUR CLOSED (branch checkpoint/physical-frontier-verification-23 from 0955c87; local main untouched at 668763f, remote main recorded and untouched at 0a43083). PRODUCTION BEHAVIOUR CHANGED — no fingerprint parity to 0955c87 is claimed or required. **CORRECTION-23I gated temporary_use on "a patch is remembered here OR some party is away toward this tile". Neither proves a camp-requiring operation is pending: memory is not intent, and a `returning` party has already taken the decision the answer was meant to inform.** New typed identity src/sim/agents/pendingOperation.ts (operationId/bandId/activityKind/targetTileId/selectedDay/expectedLaunchDay/expectedOperatingDay/requiresMultiDay/requiresTaskCampDecision/partyOrTaskIdentity/authoritativeSelector), every field read off the expedition record the production selector wrote; frontier_exploration is excluded (no destination) and frontier_verification is excluded (self-reference). Only `prepared` and `outbound` are pre-camp-decision — `operating` is already too late. **§7 ORDERING MODEL IS C, MEASURED NOT ARGUED.** Model A does not exist: maybeLaunchExpedition selects and launches in the same call, so no operation is ever selected-and-waiting (4,186,352 refusals for no_selected_operation against 27 that found a genuinely pending one). Model B does not exist: taskCampRefusedByEvidence has exactly ONE production reader and it runs on ARRIVAL, never in candidate selection. Model C holds by arithmetic — a camp falls legDays after departure while the answer needs 2*legDays + VERIFICATION_ON_SITE_DAYS; 25 of those 27 failed exactly there (fixture J4/J8 on a real route: camp in 3 days, round trip 8 days). **§10 A-H MEASURED SEPARATELY, and D=0 IS THE FINDING.** Before arm (23I gate, 11 worlds x 5 seeds x 40y): A=343 camps blocked, B=63 records, C=1,145 launches, **D=0**, E=0, F=0, G=0, H=1,145. Not one launch had a selected operation at the same tile with its camp decision still ahead; only 378 of 1,145 (33%) were even at a place work ever reached. A is split by who was refused — 343 for real work operations, 0 for verification parties themselves — so the 23I total is not inflated by the question governing its own parties. After arm: every column 0, launches 1,142 -> 0. **THE 18.09% vs 10.6% DISCREPANCY IS WORSE THAN REPORTED AND IS CORRECTED.** Running 23I's own audit UNMODIFIED on the commit its evidence shipped in (0955c87) gives **343/3,672 = 9.34%**, and a second seed prefix gives 363/3,816 = 9.51% — a 4% spread, so neither 10,724/59,286 (18.09%) nor 492/4,626 (10.6%) is inside seed jitter. The 23I evidence file describes an intermediate dirty-tree state, not committed behaviour; it could not have come from dc08b2d either (the diagnostics module did not exist there). The 10.6% figure is REMOVED from source; the 23I evidence file is PRESERVED unaltered and corrected in docs/evidence/correction23j/FINDINGS.md. **§13 OUTCOME B — temporary use suspended until a real operation reader exists**, and deliberately NOT by adding the name to SUSPENDED_QUESTIONS: the reader is real and a held negative does refuse a camp (I5, J7). The dormancy is a physical consequence of the §6 gate, so the question re-opens by itself the moment a pre-operation seam exists, with no rule to revisit. Type, resolver, evidence shape and reader all retained and documented dormant. J1-J12: **11 PASS, 1 VACUOUS (J12, recorded as vacuous NOT as a pass — there are no natural launches, so it states the contract without demonstrating the behaviour), 0 FAIL**. J2 was rewritten after a first version passed on an empty candidate list. I5's stale caveat corrected; I6 rewritten per §14 to assert that a band holding BOTH a remembered patch AND a returning party still does not ask. Closure fixtures still 14/14. §11/§12 natural matrix (11 worlds x 5 seeds, 40y and 200y, BOTH arms re-measured — the 23I acceptance files were not reused): water_access 103 -> 109 (the gate's code is untouched; the +6 is downstream world divergence from 1,142 fewer parties), three suspended questions 0 -> 0, exploration materially unchanged (most worlds identical at 13 parties; site_B -2 and site_D +2.6 move in opposite directions), no exploration eligibility/target/cadence rule touched. PASSED: tsc, build, graph 221/764 0 dup 0 dangling, import/adaptation/decision boundaries, anti-omniscience all-zero (C3 lost-party transfer 0), hidden-truth zero unsupported copies, food capture 1.000, per-lineage population and cohort conservation, determinism true, fresh-process determinism (map1 7239c085… identical across processes), step-mode invariance both maps with fullCanonicalStateMatch. State bounded. NOT DONE, deliberately: no ordinary-exploration repair, no reservation seam invented, no next-roadmap system built early, no population tuning. **The ordinary-exploration deficit remains the next blocker.** See docs/evidence/correction23j/FINDINGS.md. Previous context: DECISION-CONTINGENT VERIFICATION / VALUE-OF-INFORMATION — CORRECTION-23H is PROGRESS — DIAGNOSIS ACCEPTED / CORRECTION-23 PARENT STILL NOT MERGEABLE (branch checkpoint/physical-frontier-verification-23 from ff48d29; local main untouched at 668763f, remote main recorded and untouched at 0a43083). DIAGNOSTIC ONLY — NO PRODUCTION BEHAVIOUR CHANGED; diagnostics-off fingerprints identical to ff48d29 (map1 9a204dde…, map2 439b4e7a…). **CORRECTION-23G showed verification answers are inert in aggregate; 23H shows WHY, question by question.** THE QUESTION-TO-READER LEDGER, read from production: water_access -> isWaterAccessFeasible -> the consideredAsTarget GATE (carryingCapacity.ts:905); resource_presence -> resourceTestEligible -> ONLY the verification selector's own resource_test_possible gate (frontierVerification.ts:317); resource_test_possible -> NO READER; temporary_use -> taskCampRefusedByEvidence -> the bounded task camp (expedition.ts:293), NEGATIVE BRANCH ONLY; seasonal_persistence -> NO READER. **resource_presence -> resource_test_possible is a chain that terminates in nothing.** **temporary_use is an asymmetric reader**: absence of evidence already permits the camp, so its positive branch cannot enable anything. §5 same-snapshot Q0-Q3 counterfactual over **1,632,900 candidates** (11 worlds x 5 seeds x 40y), 99.25% baseline soundness: water_access 84.4% eligibility-relevant but **the relevance is 76.2% in the NEGATIVE arm and only 9.3% in the confirmed arm**, while realized answers are 6,414 confirmed vs **102 negative (confirmation rate 0.98 — TAUTOLOGICAL)** and **89% of candidates already have observed water above the gate**; resource_presence 43.3% eligibility-relevant with **zero negatives ever returned across 22,205 answers (rate 1.00 — TAUTOLOGICAL)**; resource_test_possible **95.4% future-system evidence**; temporary_use 69.4% eligibility-relevant carried **100% by the negative arm and 0% by the positive**, realized negatives 52%; seasonal_persistence **100% future-system, 9,435 answers all inconclusive by construction**. Immediate action relevance never exceeds 1.5% in any world; on the isolated-marginal tier 82.7% of everything asked is future-system evidence. §11 bounded-horizon reader trace (55,274 returned answers, 90-day horizon): only **6,279 (11.4%)** changed a reader that gates a physical action, and **94% of those are temporary_use negatives** (5,883); water contributes 396 (6%); resource_presence contributes **0 physical** while moving its declared reader 22,205 times; resource_test_possible and seasonal_persistence contribute **0**. H1-H12 controlled fixtures: **9/12 pass, and the 3 failures are the findings** — H3 and H5 name missing readers, H6 names the asymmetry. Gates 10 and 11 are met (H1/H12 positive changes a real gate; H2/H7 negative prevents a real action). §10 selector-only is **zero BY TAXONOMY** (a hypothetical negative moves the water gate first) and is reported with that reason rather than as a measurement. **§12 RECOMMENDATION IS PER QUESTION, NOT GLOBAL**: water_access -> A (decision-contingent launch gate); temporary_use -> E (already decision relevant, leave alone); resource_presence and resource_test_possible -> B (missing-reader construction); seasonal_persistence -> C (retire until seasonal scheduling exists). **THREE INSTRUMENT BUGS WERE CAUGHT AND ARE RECORDED**, each of which would have produced a vacuous pass: the audit re-ran the opportunity reader with cache:undefined so it evaluated a smaller candidate set (0/331 sound before the fix, 99.25% after); the reader trace counted "consumed" whenever a pure function returned a value (100% consumption for questions with NO reader); and **the Q0 arm stripped only band.verificationEvidence while find() consults KnownTileRecord.verificationDisposition FIRST** — CORRECTION-23D's place-record authority — so Q0 was not "no evidence" and the entire first matrix was discarded and re-run. §15 AUDIT-INSTRUMENTATION DEBT INVENTORIED: 39/3008 marker lines in expedition.ts, 12/1551 in carryingCapacity.ts, 11/690 in memoryCompression.ts, 10/498 in frontierVerification.ts, 6/449 in tileObservation.ts, plus 1,079 lines of diagnostics modules; **WorldAuditOptions.retentionInteractionArm has NO consumer anywhere and should be deleted**; the parent branch cannot become merge-ready until the superseded 23E/23F replay arms are removed. PASSED: tsc, build, graph 221/764, boundaries (back edges 84 unchanged), anti-omniscience all-zero, hidden-truth zero copies, lost-party no-transfer, food capture 1.000, conservation, determinism true, step-mode both maps, diagnostics-off parity. 30.90 ms/tick. NOT DONE, deliberately: no launch gate, no question disabled, no selector or score changed. See docs/evidence/correction23h/FINDINGS.md. Previous context: EXACT TRAVEL REPLAY / TARGET-SELECTION ISOLATION / TERRAIN-SENSITIVITY SIGNATURE — CORRECTION-23G is PROGRESS — DIAGNOSIS ACCEPTED / CORRECTION-23 PARENT STILL NOT MERGEABLE (branch checkpoint/physical-frontier-verification-23 from ca9e3b8; local main untouched at 668763f, remote main recorded and untouched at 0a43083). DIAGNOSTIC ONLY — NO PRODUCTION BEHAVIOUR CHANGED; every new seam is an audit-only module slot in src/sim/diagnostics/verificationScheduleReplay.ts or a counter that no-ops when unregistered, and the diagnostics-off fingerprint is identical to ca9e3b8 on map1 (9a204dde…) and map2 (439b4e7a…) at 40y. **CORRECTION-23F's gate 10 is now MET and the answer is NEGATIVE: VERIFICATION SEMANTICS CONTRIBUTE NOTHING.** G1 replays F1's exact donor schedule — same launch days, same targets, same routes tile for tile (0 rebuilds, 0 failures across 6 sites × 5 seeds) — carrying no question, no answer, no evidence and no disposition (proved: 0/0/0 rows against F1's 12/48/430). **G1 reproduces F1 EXACTLY on all six sites**, and on terrain A seed s1 at 200y matches to four decimals on every quantity measured: 32 people, support 1.3028, 97 births, 99 deaths, 226.74 receipts, 502 moves, 921 evictions, 29,163 refreshes, 326 tiles. **430 durable verification conclusions change no birth, no death, no calorie and no move.** Their whole causal contribution runs through TARGET SELECTION. **The cadence is not the mechanism either**: holding F1's exact launch days and party count and changing only the target rule collapses terrain A from 1.00/34.0 to 0.20/5.4 (G3 exploration targets), 0.40/13.6 (G4 nearest uncertain) and 0.40/19.6 (G5 rotating sectors). **Nor is target DIVERSITY**: the §6 supplement reproduces 23F's F13 (5,069.8 parties, 30.2 targets, 55.4 tiles, 0.00) and shows bounded rotation repairs the collapse completely (285.8 parties, 221.4 targets, 234.2 tiles) while still returning 0.00. What works is replaying the SPECIFIC target set F1's own band-known selector produced. G1 vs G2 is null BY CONSTRUCTION (0 rotation retargets under an exact replay) and is reported as such, not as a measurement. **G6 (retain the donor places, launch nothing) is a PARTIAL substitute on terrain A only**: F0 0.60/19.0 → 0.80/22.0, with travel worth the remaining → 1.00/34.0; worth nothing on the other five sites. **NO REPLICATED TERRAIN-CONDITIONAL MECHANISM WAS FOUND, and this is stated plainly.** Six qualified sites, two per structure class: only A is sensitive, and its class does NOT replicate — site D (tile:119:116) has MORE water (0.367 vs 0.303) and a LARGER aquatic food share (0.367 vs 0.246) and is neutral on every arm; both dry-plains and both hills sites are neutral. The two axes on which A is extreme make it an OUTLIER, not a type: it is the only site with a FREE corridor (100% passable, 0 obstacles; every other site has 1–3) and the only site whose production baseline is not already safe (F0 0.60 vs 0.96 mean elsewhere). **TERRAIN A FAILS THE QUALIFICATION RULE 23F ITSELF WROTE** (corridorPassableShare ∈ [0.55,0.98]; A is 1.00) — 23F used as its reference a site that fails its own filter and never checked. Do not treat A as a representative marginal_escapable fixture without re-qualifying it. **§8: EVERY new record necessarily received base observation content (ratio exactly 1.00 on every arm and site), so a "new tiles only" arm can never be a season-only arm** — 23F's F5/F10 must not be read as clean season-identity tests, and this pass claims none. seasonsObserved has exactly four behavioural readers; the two verification-side ones fall to 0 on G1–G5 while the only direct movement-scoring consumer (bandDecision's 0.06 seasonal-food-modifier penalty) is IDENTICAL between F1 and G1. Complete §13 mediation chains for every material arm. PASSED: tsc, build, graph 221/764, import/adaptation/decision boundaries (back edges 84, unchanged), anti-omniscience all-zero, hidden-truth zero copies, lost-party no-transfer, food capture 1.000, population and cohort conservation, determinism true, step-mode both maps with fullCanonicalStateMatch, diagnostics-off parity. State bounded (disposition rows/band 174→397→415→490 at 25/50/100/200y; evidence pinned at 48). 31.13 ms/tick. NOT DONE, deliberately: no production behaviour selected, restored, tuned or changed. See docs/evidence/correction23g/FINDINGS.md. Previous context: SEASONAL RETRAVERSAL BENEFIT DECOMPOSITION — CORRECTION-23F is PROGRESS — THE CORRECTION-23E GENERALISATION IS REFUTED / GATES 9, 10, 11, 16 UNMET (branch checkpoint/physical-frontier-verification-23 from a5b67a0; main untouched at 668763f). DIAGNOSTIC ONLY — NO PRODUCTION BEHAVIOUR CHANGED. **The 23E season-term and route-observation results are TERRAIN-A-SPECIFIC and their general claim is WITHDRAWN.** Across three qualified marginal_escapable terrains: terrain A (tile:204:72, coast) F0 0.70/21.5 vs F1 1.00/36.9; terrain B (tile:10:34, plains) every arm but F0/F7 survives 5/5 inside 8.4-13.8; terrain C (tile:100:23, plains) EVERY arm survives 10/10 and F1 is the WORST at 21.4 against production 24.6. 23E used ten runSeed values on ONE site, which vary near-tie ordering and never terrain. **Do not build a production seam from the 23E result.** On terrain A the benefit is an INTERACTION: route country not destination (F4 1.00/34.6 reproduces F1; F3 target-only 0.30/6.5 is BELOW the no-observation control 0.40/12.1), neither discovery nor maintenance alone (F5 0.00/0.0, F6 0.30/10.5), neither content nor recency alone (F7 0.80/23.4, F8 0.10/3.5), season identity necessary but not sufficient (F9 0.70/20.1 = production, F10 0.10/1.8). **F13 is INVALID as run — do not cite its 0.00**: suppressing the result also suppresses the disposition, so retry memory is lost and selection collapses (5,081 parties, 55.6 unique tiles). Memory-compression debt is interaction-dependent, not the principal mechanism: the 161%-of-capacity mandatory set is identical on all three terrains while only A is sensitive. NOT BUILT: F11, F12, F14, F16; F15 subsumed by F7. See docs/evidence/correction23f/FINDINGS.md. Previous context: RETRY-SUPPRESSION POPULATION MEDIATION / PLACE-MEMORY RETENTION AUTHORITY — CORRECTION-23E is PROGRESS — DIAGNOSIS ACCEPTED / CORRECTION-23 PARENT STILL NOT MERGEABLE (branch checkpoint/physical-frontier-verification-23 from 6258c97; main untouched at 668763f). DIAGNOSTIC ONLY — NO PRODUCTION BEHAVIOUR CHANGED; every new switch is an audit-only WorldAuditOptions field. **The CORRECTION-23D marginal regression is ONE deleted line.** Arm R2 (durable disposition still WRITTEN, pre-23D eligibility READ) reproduces 76893be EXACTLY seed-for-seed, so the storage/authority change is innocent; arm R4 (23D gate + only the removed `seasonChanged && (water_access || resource_presence)` term) gives 1.00/36.9 against production 0.70/21.5 and R0 0.90/35.6. Hardship removal is NOT the cause (R3 0.60/18.7). Disabling settled-answer suppression gives the MOST launches and the SECOND WORST population (0.70/14.1) — **more verification is not better**. **Verification's survival value is its WALK, not its answer**: arm R6 suppresses only the ordinary tile observation of the walked route and survival falls 0.70 -> 0.10, population 21.5 -> 2.1. **The memoryCompression retention scoring is INERT in production** — the mandatory set (position + full 2-ring + crossings + important water + valenced places) is 161% of the 72 capacity on the marginal fixture and 113% on default map2, and mandatory records sort first, so `retained.size < capacity` is never true for a scored record. Recency/visits/confidence/water/provenance change NOTHING; route, candidate and verification relevance are not terms at all. **Forgetting fails the spec's own legitimacy test**: 96.9% of evicted records were observed within the previous year, 92.2% are reacquired, and 250 verification questions are re-asked after reacquisition (1,493 on default map2). **DO NOT repair this by adding memory** — K4 (capacity 72 -> 288) collapses the marginal tier to 0.20/7.6 and K5 (no mandatory set) to 0.10/3.7, while K1-K3 (protect verified places) remove 96-97% of the re-verification loop and buy +2 people. K4/K5 are NOT attributed. The regression is confined to marginal_escapable: both default maps, ordinary and hostile are within ±2%, and the isolated tier is identical in all arms. CORRECTED from the CORRECTION-23D report: the isolated survivor is at population ELEVEN (1.1 was the ten-seed mean), and the ten "shared seeds" are ten runSeed values on ONE site (runSeed perturbs near-tie ordering only, never terrain). See docs/evidence/correction23e/FINDINGS.md. Previous context: FRONTIER OPPORTUNITY / DAUGHTER FISSION CAUSAL CLOSURE — CORRECTION-20 is PROGRESS — NOT ACCEPTED / DO NOT MERGE (branch checkpoint/frontier-opportunity-fission-closure-20; main untouched at 668763f). NO PRODUCTION BEHAVIOUR CHANGED. **THE FISSION HYPOTHESIS IS REFUTED — BOTH MAPS ARE NON_FISSION_DOMINATED.** Reader isolation (5 seeds x 2 maps x 4 arms) gives map1 fission-only +7.0 / non-fission -60.2, and map2 fission-only -4.8 / non-fission -30.4. Frontier knowledge reaching opportunity/fission is harmless-to-helpful; the loss comes from the NON-FISSION readers (movement, resource selection, camps, seasonal rounds). **Do NOT tune the travelCost/split-motivation coupling** — the §9 ledger records it as a real contract violation (T2 re-consumes T1 with no physical quantity of its own) but it is NOT the mechanism. **The expedition is close to costless**: ARM_A (walks, commits workers, transfers nothing) matches or BEATS disabled — 241.6 vs 244.6 and 231.6 vs 226.0 — with band counts restored. §5 completed: map1 gap is 88.8% amplification, map2 95.7% direct; these say WHERE the difference appears, §6 says WHICH READER causes it. **RETRACTED**: the CORRECTION-19 claim that exploring bands are better fed (+12.4% support) came from ONE seed; across five, support is -0.33% (map1) and -9.09% (map2). Leading unrepaired mechanism: KnowledgeAcquisitionKind provenance exists but no adequacy test reads it, so single-traversal tiles carry full residential confidence into movement/resource/camp decisions. Previous context: FRONTIER EXPEDITION PHYSICAL-COST AND LABOR ACCOUNTING — CORRECTION-19 concluded that the expedition labour accounting is **CORRECT AND SINGULAR** and NO REPAIR is warranted at the labour seam (branch checkpoint/frontier-expedition-labor-accounting-19; main untouched at 668763f). The FEATURE BRANCH overall stays PROGRESS — NOT ACCEPTED / DO NOT MERGE. **§12 invariants: ZERO violations over 9,600 sampled days** — no double-commitment, no over-reservation, no reservation outliving the journey, labour released on terminal phase, every frontier party exactly two workers (399/399), person-days STEP-MODE INVARIANT (8807=8807, 10343=10343). **Two paths UNDER-charge and none double-charges**: adultEquivalentDemand counts away adults (correct, they eat), laborCapacity also counts them (under-charge), party sizing subtracts awayWorkers once and scales the PARTY not the band. **Provisions cost the band ZERO food** — never deducted from the ledger, and for information-only tasks buildReturnedRecord never even runs. **Expedition walking imposes NO whole-band fatigue** — getRecentMovementFatigue reads RESIDENTIAL moves. **THE REGRESSION IS NOT A FOOD OR LABOUR COST — do not describe it as one.** Normalized per §5, the two maps differ: map1 pop/band ON 28.85 vs OFF 27.65 with FEWER BANDS (7,8,6 vs 10,8,9); map2 band counts IDENTICAL with a real per-band effect. On map1 c18:a food per working-adult-year is identical (0.0050 vs 0.0049), the raw support ratio is HIGHER with exploration (0.3019 vs 0.2686), expedition labour is 0.32% of working-adult days (frontier-specific ~0.12%), and the gap is fissions: 2 @ y102 vs 5 @ y80. Leading mechanism, recorded not repaired (fission out of scope): CORRECTION-18 §9.3 distance DOUBLE-COUNT between destination ranking and split motivation. NOT RUN: Arms 0-7 matrix, §7 external-divergence successor, most of §16. INHERITED AUTHORSHIP: d41c973 has author/committer name "Claude" + trailer, 1faa7c9 has the trailer; both predate the rule and were REPORTED NOT REWRITTEN (rewriting destroys the required ancestry). Previous context: FRONTIER KNOWLEDGE CONSUMPTION / DAUGHTER-DESTINATION VIABILITY — CORRECTION-18 is PROGRESS — NOT ACCEPTED / DO NOT MERGE (branch checkpoint/frontier-knowledge-consumption-destination-18 from febbdc2; main untouched at 668763f). DIAGNOSTIC half done; construction half largely NOT done. **(1) The population regression REPRODUCES and is larger than reported**: 5 predeclared seeds/map, map1 -23.98%, map2 -15.13%, 5/5 seeds lower on both. The trip loss is a CONSEQUENCE — per-band trips are HIGHER with exploration on; totals fall only because there are fewer bands. **(2) §6 first divergence = EXPEDITION LABOUR on 6/6 runs** (lockstep DAILY FROM TICK ZERO; a 40-year seasonal warm-up had reported everything on day 1, a pure sampling artefact): party raised t96 -> walks t97 -> knowledge home t106 -> support t192 -> fission pressure t360 -> demography t720. **(3) §7 Arm A** (frontierKnowledgeTransferDisabled: party runs physically, only the return hand-off suppressed) on one seed: production 215 / armA 209 / disabled 264 => labour ~112% of the gap, returned knowledge slightly POSITIVE. NOT the required five-seed matrix; C1-C9 not built. **(4) §11 MASKING CONFIRMED AND REPAIRED — this CORRECTS CORRECTION-17.** Over 50,579 production ledgers, frontier-derived candidates are NOT starved (62,544 reach the list, 19,773 viable, distances to 41 tiles evaluated), but the score winner FAILS viability 63.7% of the time and 6,413 viable candidates were discarded because a non-viable one scored higher. CORRECTION-17 measured only the score winner, so its "no alternatives materially better / blocker is ecological" is NOT SUPPORTED — do not cite it. Repair: deriveKnownUnusedHabitat keeps a best-VIABLE slot ranked by the SAME score and returns bestViable ?? best; no threshold or coefficient moved; step-mode invariance still passes with full canonical state match. **(5) §9 units ARE incommensurable** (per-tile yield fraction vs catchment-support/band-demand ratio; candidate side understated ~5-6x) **but are NOT the blocking gate** — a derived hypothesis that clamp01 makes the test unsatisfiable was REFUTED: consideredAsTarget is true in 87.5% of band-years. **(6) §9.3 distance is DOUBLE-COUNTED** in destination ranking AND split motivation, so discovering good distant country makes a band LESS willing to divide (structural; magnitude unmeasured; unrepaired). **(7) §8 typed KnowledgeAcquisitionKind provenance implemented** and stamped at the return seam. NOT BUILT: §12 synthetic cases, §13 regression repair, §14 memory bounds + 500/1000y proofs, §16 acceptance matrix, §19 performance. Previous context: CORRECTION-17 is PROGRESS — NOT ACCEPTED / DO NOT MERGE (branch checkpoint/frontier-exploration-knowledge-horizon-17 from 1faa7c9; main untouched at 668763f). **The ~9-tile destination-knowledge horizon that CORRECTION-14/15/16 all named as the binding blocker is REMOVED as a limit.** A fifth expedition task family `frontier_exploration` (src/sim/agents/frontierExploration.ts) walks out on a band-known DIRECTIONAL hypothesis — no destination tile, no remembered patch, no precomputed route — discovering its route one 4-adjacent physical step at a time, reserving return capacity at every step, holding observations party-local until it physically walks home, and writing residential knowledge only through the canonical observeTileAndNearby writer. Residential max known distance measured 40.4 tiles ENABLED vs 28.8 DISABLED on identical seeds (single-founder baseline was 7-11 across 300 years). Anti-omniscience PASS: 622 breadcrumb steps all 4-adjacent, 0 party-local leaks, 0 lost-party transfers, 0 resource memories or food receipts created by observation, 0 unknown heading anchors. Lost-party control arm (129-152 parties lost, 0 completed) transfers nothing — this is the strongest evidence in the checkpoint. **The chain still does NOT close**: it breaks at L09 (opportunity evaluation over the new country) on 5/5 seeds. §15 was ANSWERED with measurement: non-overlapping candidates are known, admitted, and survive the candidate slice (795/795 band-years), but never win; across 7,186 such candidates, ZERO held a habitat advantage exceeding their travel penalty and lost anyway. A strictly superior synthetic region still lost because the parent catchment's own observed richness is already 0.93-1.0. **Do NOT tune the travel-cost term on this evidence.** A real candidate-budget defect in collectOpportunityCandidates was isolated, a repair was implemented, measured to change no outcome, and REVERTED — it is documented debt, not a fix. OPEN AND BLOCKING FOR MERGE: a default-map population A/B effect. Previous context: CORRECTION-16 is PROGRESS (evidence repair done, construction scope unbuilt). Method rules are binding: docs/evidence/correction16/AUDIT_ADMISSIBILITY.md. CORRECTION-15 was PARTIAL and two of its conclusions are RETRACTED. DONE and measured: (1) all four CORRECTION-14 candidate repairs independently re-proven on this branch before porting — annual demography consumed 0.555 mean food pressure against a year that held 0.335 (89 physically-surplus years, 0 surplus-signal years); 31 of 480 seasons had zero trips while only out-of-budget memories existed; a saturated 48-slot RESOURCE_KNOWLEDGE_CAP evicts the just-observed local patch (unit proof); and the expedition timestamp repair is REQUIRED BY the other two (668763f passes step-mode only because the recon path is never exercised). (2) Recovery basin is sound — no absorbing collapse spiral: one/three/five-severe bad years all recover, chronic hunger clears in 2 years, bereavement washes out, small initial differences do not bifurcate. (3) Cohort composition magnitude is **UNRESOLVED** — the "0.01 of support ratio / labor collapse not reachable" figure is RETRACTED BY CORRECTION-16 pending re-measurement. (4) **The social layer is NOT readability-only — CORRECTION-16 disproved this.** The C15 clamp was applied BETWEEN ticks, but innerFission/socialTension are rewritten by applyInnerFissionSocialReadabilityContext at position 7 of the updateBandContextStates chain BEFORE their readers (applyProtoCampContext at 8, applyForagingLearningAdaptationContext at 12, pressure.ts via bandDecision) execute — the perturbation never reached a reader. Perturbed at the correct seam, innerFission changes movement, physical receipts, knowledge, demography and viability on 5/5 seeds; socialTension and cohesion on 4/5. You MAY cite these as causes, naming the traced path. Still do NOT add a generic cohesion/cooperation scalar. NOT BUILT: the whole-viability cause taxonomy, the adaptation cascade, and the dedicated multi-cause extinction arms. The destination knowledge horizon (a band never learns country beyond ~9 tiles) remains the binding limit on multi-generation expansion. CLIMATE / WEATHER / REGIONAL SEASONALITY FOUNDATION-2 remains the standing roadmap item after these.
