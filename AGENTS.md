...
Last updated: 2026-07-25 (REPEATED BAND EXPANSION + HABITAT VIABILITY CORRECTION-14 — PROGRESS, DO NOT MERGE, on branch checkpoint/repeated-band-expansion-fission-14 from public main 668763f. CORRECTION-13 was integrated into main at the start of this checkpoint; local and remote main both equal 668763f.)
Current active checkpoint: DESTINATION KNOWLEDGE HORIZON / EXPLORATION REACH — named by CORRECTION-14 as the measured blocker for repeated multi-generation expansion. CORRECTION-14 (PROGRESS, not accepted) corrected three defects in the expansion chain: (1) the ANNUAL demographic step read the INSTANTANEOUS nutrition state, so it sampled the same seasonal trough every year — richest map2 catchment averaged 1.73x demand while the demographic read logged 0.79 mean food stress, with 359 physically-surplus years and 1 surplus-signal year; fixed by `deriveAnnualNutritionState` (seasonalSurvival.ts) consumed by the single annual call site in demography.ts, with the seasonal read retained for every behavioral consumer. (2) `selectTripCandidate` ran its argmax over every distance while `applyTripDay` discards out-of-budget winners, wasting the band's ONE candidate per trip day — fixed with a `requireSameDay` domain (the mirror of CORRECTION-4's expedition fix). (3) A saturated 48-slot `RESOURCE_KNOWLEDGE_CAP` evicted every newly observed patch, so a band that relocated could never learn local ground — fixed by protecting just-observed patch ids in `enforceResourceKnowledgeCap`; local reconnaissance now bootstraps on no REACHABLE memory. Zero-support seasons fell 141/480 -> 1/480; rich lineages 0 -> 4 median successful fissions; Map 1 defaults 0 -> 8 fissions and 65 -> 337 people. STILL UNMET: no second-generation fission, no rich seed with 6 successful fissions, `good` majority fission, marginal escape, hostile extinction — four of five traced to a measured 9-tile destination-knowledge horizon (0 known tiles beyond 10 at every sample to y300), which confines every daughter to the parent's own catchment. CLIMATE / WEATHER / REGIONAL SEASONALITY FOUNDATION-2 remains the standing roadmap item after it.
...
| **500-year lineage expansion chain (CORRECTION-14): habitat tiers from measured physical evidence, full genealogy, per-year support/nutrition/demography/fission-pressure series, fission-evaluation gate values, daughter-outcome classification, terminal-blocker taxonomy, population + cohort conservation. `--mode sites\|tiers\|default\|arm-b --tier <name>\|all --map --years --seeds --arm production\|no-fission --out`** | `lineageExpansionAudit.mjs` |
| **Canonical per-band state fingerprint (diagnostics-off parity vs a parent-commit worktree; fresh-process determinism). `--map --years [--seed]`** | `canonicalStateFingerprint.mjs` |
| **Food-access collapse probe (per-season trips/receipts/failure reasons/accumulator freshness; prints the windows around every zero-support season)** | `foodAccessCollapseProbe.mjs` |
| **Destination knowledge horizon probe (distance distribution of a founder's own known tiles; the measured blocker for multi-generation expansion)** | `destinationKnowledgeHorizonProbe.mjs` |
| **Expansion performance + retained-state bounds (ms/tick and serialized band state for both default maps at 150y and 500-year rich/ordinary/hostile lineages; run ALONE). `--case default\|lineage\|all`** | `expansionPerformanceProbe.mjs` |
