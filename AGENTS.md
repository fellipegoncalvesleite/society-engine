...
Current active checkpoint: HUMAN VIABILITY / RECOVERY / ADAPTIVE RESILIENCE — CORRECTION-15 is PARTIAL. DONE and measured: (1) all four CORRECTION-14 candidate repairs independently re-proven on this branch before porting — annual demography consumed 0.555 mean food pressure against a year that held 0.335 (89 physically-surplus years, 0 surplus-signal years); 31 of 480 seasons had zero trips while only out-of-budget memories existed; a saturated 48-slot RESOURCE_KNOWLEDGE_CAP evicts the just-observed local patch (unit proof); and the expedition timestamp repair is REQUIRED BY the other two (668763f passes step-mode only because the recon path is never exercised). (2) Recovery basin is sound — no absorbing collapse spiral: one/three/five-severe bad years all recover, chronic hunger clears in 2 years, bereavement washes out, small initial differences do not bifurcate. (3) Cohort composition is causal ONLY through task-group party size and is worth 0.01 of support ratio — a labor-collapse extinction cause is NOT reachable. (4) The social layer is READABILITY-ONLY in practice: clamped every tick for six years, cohesion at 0.02 AND 0.99, innerFission at near_split and socialTension at 0.95 all leave canonical state byte-identical; only socialPressure is causal and it is written by applyDemographyToSocialPressure (demography restated socially). Do NOT cite cohesion or innerFission as an explanation for survival or fission. NOT BUILT: the whole-viability cause taxonomy, the adaptation cascade, and the dedicated multi-cause extinction arms. The destination knowledge horizon (a band never learns country beyond ~9 tiles) remains the binding limit on multi-generation expansion. CLIMATE / WEATHER / REGIONAL SEASONALITY FOUNDATION-2 remains the standing roadmap item after these.
...
| **Social causality (CORRECTION-15 §10): static production-vs-projection reader map + extreme-value perturbation CLAMPED EVERY TICK; classifies each social field causal vs readability-only** | `socialCausalityAudit.mjs` |
