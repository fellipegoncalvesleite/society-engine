...
Current active checkpoint: RESOURCE INVESTIGATION PHYSICAL EXECUTION — CORRECTION-26 (branch checkpoint/resource-investigation-physical-26, continuing its own architecture-decision commit b746b68; CLOSURE-25 FROZEN at f947550; local and remote main untouched at 0a43083). **PRODUCTION BEHAVIOUR CHANGED — no fingerprint parity is claimed or possible.** Entry invariant PASSED: `git diff --exit-code f947550 -- src/sim` clean. **A selected `resource_scout`/`logistical_probe` no longer observes anything.** `collectProbeObservationTargets` is deleted; selection observes what a `stay` observes and leaves ONE bounded `PendingInvestigationRecord` carrying the exact `Decision.id` into the following season's first trip day, where `agents/intraSeasonTrips.ts` staffs a party from the labour the day's foraging group left, walks the same `buildOutboundPathTiles`/`findPassablePath` route, and either arrives or names why not. Observation happens ONLY on arrival, through the canonical `observeTileAndNearby`. **Measured at the pre-existing audit-only `decisionObserver` seam (unchanged at f947550): target-area knowledge gained at selection 176/192 (91.7%) → 0/234, with 234/234 now carrying an exact execution identity — on MORE selections, not fewer.** 343 natural selections resolve 139 executed / 147 `beyond_same_day_reach` / 54 `route_unavailable` / 3 pending / 0 lost / 0 duplicate executions / 0 information receipts. `TemporaryTaskCampRecord` → `TemporaryTaskPartyRecord`, written only on a real departure (`camp_movement_temporary_record` 129 → 0; `expedition_task_camp` 103 → 113, untouched and NOT merged). 0 runtime cycles in `src/sim`, 0 `agents → rules` runtime edges. **A real step-mode regression was introduced and fixed** (the executor observed with `runDailyActions`' span-start `world.time`) and the fixture that passed while it was live was strengthened with a negative control. **INHERITED, NOT A REGRESSION:** `expeditionLifecycleAudit` FAILS here and identically on f947550. **DEFERRED AND UNPROVEN:** the 10-tile selection reach vs the 8-tile same-day budget refuses 43% of investigations; whether that is a defect or an honest limit is NOT established and no constant was touched. NOT RUN: long-horizon matrix, population/survival comparison, Crowding. See docs/evidence/resource-investigation-physical-26/FINDINGS.md.
...
| **CORRECTION-26 controlled fixtures P1–P13 (successful scout/probe, blocked route, insufficient labor, same-day and multi-day boundaries, latency, cancellation, no duplicate execution, no false party record, zero receipt, step-mode parity incl. observation timestamps)** | `investigationPhysicalFixturesAudit.mjs` |
